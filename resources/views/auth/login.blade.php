...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tabForms = tabs.map(tab => document.getElementById(`${tab}-form`));
    ...
    tabButtons.forEach((button, index) => {
        button.addEventListener('click', () => {
            ...
            // Set specific colors for each tab
            if (index === 0) { // Admin
                button.classList.add('bg-white', 'text-red-600', 'shadow-sm');
            } else if (index === 1) { // Doctor
                button.classList.add('bg-white', 'text-green-600', 'shadow-sm');
            } else { // Patient
            ...
            // Update forms
            tabForms.forEach(form => form.classList.add('hidden'));
            tabForms[index].classList.remove('hidden');
        });
    });
    ...
    const patientPasswordLogin = document.getElementById('patient-password-login');
    const patientOtpLogin = document.getElementById('patient-otp-login');
    ...
    showOtpLogin.addEventListener('click', function() {
        patientPasswordLogin.classList.add('hidden');
        patientOtpLogin.classList.remove('hidden');
    });

    showPasswordLogin.addEventListener('click', function() {
        patientOtpLogin.classList.add('hidden');
        patientPasswordLogin.classList.remove('hidden');
    });

    // Send OTP functionality
    sendOtpBtn.addEventListener('click', function() {
        const identifier = document.getElementById('patient_otp_identifier').value;
    ...
    const otpInput = document.getElementById('patient_otp_code');
